...
routerAdd('POST', '/getinstancefile', (c) => {
	const data = $apis.requestInfo(c).data
	const project_instance_id = data?.project_instance_id
	...
	if (!path) {
		return c.json(200, { data: null, error: 'path is required' })
	}
	...
	try {
		...
		try {
			res = $http.send({
				url:     `http://${sites[0].site}:5000/getinstancefile`,
				method:  "POST",
				body:    JSON.stringify({
					path: path, 
					port: sites[0].port.toString()
				}),
				headers: {
		...
		return c.json(res.statusCode, { data: res, error: null })
